...
/*  Total number of rows of the tables of the given schema is :
...
-- NULL values are present in these columns of movie table - country, worlwide_gross_income, languages, production_company. 
...
-- There were 1059 movies produced in the USA or India in the year 2019.
...
-- There are total 13 unique genres present in the dataset.  
...
-- The genre 'Drama' had the highest number of movies produced overall which was 4285.
...
-- ACtion has the highest average duaration which is approximately 112.88 minutes.
...
-- Thriller movies are ranked 3rd based on number of movies produced. 
...
WHERE movie_rank <= 10;
 ...
 -- We find that movies Kriket and Love in Kilnerry has an average rating of 10.
...
-- Total number of movies is highest with median rating 7. 
...
-- Dream Warrior Pictures and National Theatre Live has the hhighest number of hit movies. 
...
-- A total of 361 movies were released between 1 April 2018 and 1 April 2019 with median rating of 8. 
...
SELECT 
    ...
    RANK() OVER(ORDER BY COUNT(ra.movie_id) DESC) AS actor_rank
FROM names AS n
...
-- The top actors based on median rating greater than or equal to 8 are Mammootty and Mohanlal. 
...
-- The top 3 production companies are Marvel Studios, Twentieth Century Fox and Warner Bros. based on number of votes. 
...
INNER JOIN genre g
ON g.movie_id = r.movie_id
WHERE genre = 'drama' AND avg_rating > 8 and category = 'actor'
GROUP BY name
LIMIT 3;
